...
namespace App\Filament\Fodig\Resources\AnonymizationRuleResource\RelationManagers;
...
class ColumnsRelationManager extends RelationManager
{
    ...
    protected static ?string $title = 'Columns using this rule';

    public function table(Table $table): Table
    {
        return $table
            ->heading('Columns using this rule')
            ->description('Catalog columns currently assigned to this anonymization rule.')
                ...
                Tables\Actions\Action::make('attachColumns')
                    ...
                    ->form([
                        Select::make('column_ids')
                            ...
                            ->helperText('Search by database, schema, table, or column name. Columns already assigned to another rule will not appear.'),
                    ])
    ...
    private function columnSelectOptions(?string $search = null, ?array $ids = null, int $limit = 25): array
    {
        ...
        if ($ids) {
            ...
            // Exclude columns already assigned to any rule (since a column can only have one rule)
            $query->whereDoesntHave('anonymizationRule');
        }
    ...
    private function applyColumnSearch(Builder $query, string $search): void
    {
        ...
        $query->where(function (Builder $builder) use ($term) {
            $builder
                ...
                ->orWhereHas('table', fn(Builder $q) => $q->whereRaw('LOWER(table_name) LIKE ?', [$term]))
                ->orWhereHas('table.schema', fn(Builder $q) => $q->whereRaw('LOWER(schema_name) LIKE ?', [$term]))
                ->orWhereHas('table.schema.database', fn(Builder $q) => $q->whereRaw('LOWER(database_name) LIKE ?', [$term]));
        });
    }
