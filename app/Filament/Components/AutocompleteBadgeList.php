            ...
            ->content(function ($get, $livewire) use ($autocompleteOptionsFn, $context, $emptyText) {
                ...
                $badges = collect($options)
                    ->map(function ($opt) {
                        ...
                        return "<span class='klamm-badge inline-block text-xs font-semibold mb-2 px-2.5 py-0.5 rounded' style='cursor:pointer;' title='" . htmlspecialchars($tooltip, ENT_QUOTES) . "' data-reference-id='" . htmlspecialchars($copyText, ENT_QUOTES) . "' tabindex='0' role='button' aria-label='Copy selector for " . htmlspecialchars($label, ENT_QUOTES) . "'>" . $label . "</span>";
                    })
                    ->implode('');
                $scrollBox = "<style>\n"
