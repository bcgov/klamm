...
class ImportTemplateHelper
{
    ...
    private static function createField($user, $formVersionID, $field, &$elementNum, $order, $groupID = null, $containerID = null)
    {
        ...
        if (!$newField) {
            ...
            // Create selectable_value_instances
            if (isset($field['listItems'])) {
                self::createSelectableValues($user, $field['listItems'], $newField->id, $formVersionID);
            }
        }
    }
    ...
    private static function createSelectableValues($user, $selectableValues, $fieldID, $formVersionID)
    {
        foreach ($selectableValues as $index => $selectableValue) {
            // Check if SelectableValue exists, create if not
            $selectableValue = SelectableValue::firstOrCreate(
                ['name' => $selectableValue['name']],
                [
                    'label' => $selectableValue['text'],
            ...
            if (!$selectableValue) {
                Notification::make()
                    ->title('Error')
                    ->body("Failed to create SelectableValue `{$selectableValue['name']}`.")
                    ->danger()
                    ->send()
                    ->sendToDatabase($user);
            } elseif ($selectableValue->wasRecentlyCreated) {
                Notification::make()
                    ->title('Error')
                    ->body("Creating new SelectableValue: `{$selectableValue['name']}`.")
                    ->warning()
                    ->send()
                    ->sendToDatabase($user);
            }

            // Create selectable_value_instance
            $newSelectableValueInstance = SelectableValueInstance::create([
                'selectable_value_id' => $selectableValue->id,
                'form_instance_field_id' => $fieldID,
                'order' => $index + 1,
            ]);
            if (!$newSelectableValueInstance) {
                Notification::make()
                    ->title('Error')
                    ->body("Failed to create SelectableValue `{$selectableValue['text']}: {$selectableValue['value']}`.")
                    ->danger()
                    ->send()
                    ->sendToDatabase($user);
            }
